"use client";

import { ReactNode } from "react";
import {
  LayoutDashboard,
  Landmark,
  ArrowRightLeft,
  Wallet,
  ShieldCheck,
  FileText,
  Webhook,
  Users,
  Settings,
} from "lucide-react";

export type NavItem = {
  label: string;
  to: string;
  icon: ReactNode;
};

export type NavSection = {
  id: "core" | "ops" | "admin";
  label: string;
  items: NavItem[];
};

export const navSections: NavSection[] = [
  {
    id: "core",
    label: "Core",
    items: [
      { label: "Dashboard", to: "/dashboard", icon: <LayoutDashboard className="h-4 w-4" /> },
      { label: "Mint", to: "/mint", icon: <Landmark className="h-4 w-4" /> },
      { label: "Transfer", to: "/transfer", icon: <ArrowRightLeft className="h-4 w-4" /> },
      { label: "Redeem", to: "/redeem", icon: <ArrowRightLeft className="h-4 w-4 rotate-90" /> },
      { label: "Policy Rules", to: "/policy-rules", icon: <ShieldCheck className="h-4 w-4" /> },
    ],
  },
  {
    id: "ops",
    label: "Ops",
    items: [
      { label: "Wallets", to: "/wallets", icon: <Wallet className="h-4 w-4" /> },
      { label: "Activity Log", to: "/activity-log", icon: <FileText className="h-4 w-4" /> },
      { label: "Webhooks", to: "/webhooks", icon: <Webhook className="h-4 w-4" /> },
    ],
  },
  {
    id: "admin",
    label: "Admin",
    items: [
      { label: "Users", to: "/users", icon: <Users className="h-4 w-4" /> },
      { label: "Admin Settings", to: "/admin-settings", icon: <Settings className="h-4 w-4" /> },
      { label: "Settings", to: "/settings", icon: <Settings className="h-4 w-4" /> },
    ],
  },
];